...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!response.ok) {
      // Se o backend não tiver essa rota, verificar localmente por localStorage
      // Verificar se há outro dispositivo com mesmo WhatsApp logado
      if (typeof window !== 'undefined') {
        // Esta verificação será feita no cliente
        return NextResponse.json(
